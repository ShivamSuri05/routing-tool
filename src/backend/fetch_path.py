...
def fetch_paths(src, dst, height, buffer_height, num_paths):
    src = src.strip().split()
    start_lat = convert_to_float(src[0]) 
    start_lon = convert_to_float(src[1])    
    dst = dst.strip().split()
    end_lat = convert_to_float(dst[0])
    end_lon = convert_to_float(dst[1])
    buffer_height = convert_to_float(buffer_height)
    height = convert_to_float(height)
    num_paths = int(num_paths)
    print("Your string here")
    print(start_lat, start_lon,end_lat,end_lon, height, buffer_height)
    valid_filepath = "data/autobahns_germany_with_restrictions_v1.graphml"
    loaded_graph = use_saved_graph_sample(valid_filepath)
    try:
        paths = get_all_routes(loaded_graph, start_lat,start_lon,end_lat,end_lon, num_paths , buffer_height+height)
